...
var cryptoNightDark = multiHashing['cryptonight_dark'];

function cryptoNightDarkFast(buf) {
    return cryptoNightDark(Buffer.concat([new Buffer([buf.length]), buf]), true);
}
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                ...
                var blockFastHash = cryptoNightDarkFast(convertedBlob || cnUtil.convert_blob(shareBuffer)).toString('hex');
                log('info', logSystem,
